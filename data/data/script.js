...
function updateSliderPWM(element) {
    if (websocket) {
        ...
        websocket.send(sliderNumber + "rs" + sliderValue.toString());
    }
}
...
// Update the setRelayTime function to use the new validation
function setRelayTime(relayNumber, startHourElement, startMinuteElement,
    endHourElement, endMinuteElement, onOffSwitch, confirmButton) {

    if (!startHourElement || !startMinuteElement || !endHourElement ||
        !endMinuteElement || !onOffSwitch || !confirmButton) {
        console.error('Missing required elements for relay time control');
        return;
    }
    ...
    confirmButton.addEventListener('click', () => {
        ...
        sendWebSocketMessage(message, confirmButton);
    });
}

function sendWebSocketMessage(message, confirmButton) {
...
function onMessage(event) {
    ...
    try {
        ...
        // Handle non-JSON messages (like relay timer messages)
        const message = event.data;
        if (message.startsWith('relayTimer')) {
            const [command, relayNum, ...params] = message.split(':');
            if (params.length === 5) { // startHour, startMin, endHour, endMin, enabled
                updateRelayTimeUI(parseInt(relayNum), ...params);
            }
            return;
        }
    }
